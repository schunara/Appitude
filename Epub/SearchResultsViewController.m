...
@synthesize resultsTableView, objMainViewController, currentQuery, results;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [objMainViewController loadSpine:hit.chapterIndex atPageIndex:hit.pageIndex highlightSearchResult:hit];    
}
...
- (void) searchString:(NSString *)query inChapterAtIndex:(int)index{
    ...
    Chapter* chapter = [objMainViewController.loadedEpub.spineArray objectAtIndex:index];
    ...
    if(hitCount!=0){
        ...
        if((currentChapterIndex+1)<[objMainViewController.loadedEpub.spineArray count]){
            [self searchString:currentQuery inChapterAtIndex:(currentChapterIndex+1)];
        } else {
            objMainViewController.searching = NO;
        }
    }
}
...
//    NSLog(@"w:%f h:%f", webView.bounds.size.width, webView.bounds.size.height);
	...
	NSString *setTextSizeRule = [NSString stringWithFormat:@"addCSSRule('body', '-webkit-text-size-adjust: %d%%;')",[[objMainViewController.loadedEpub.spineArray objectAtIndex:currentChapterIndex] fontPercentSize]];
